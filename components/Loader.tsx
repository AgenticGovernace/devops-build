/**
 * @fileoverview Loading spinner component for async operation feedback.
 * @module components/Loader
 */

import React from 'react';

/**
 * Props for the Loader component.
 * @interface LoaderProps
 * @property {string} [text] - Optional loading message text. Defaults to 'Generating...'.
 */
interface LoaderProps {
  text?: string;
}

/**
 * Animated loading spinner with customizable text.
 * Displays a rotating SVG icon and message during asynchronous operations.
 *
 * @component
 * @param {LoaderProps} props - Component props.
 * @returns {JSX.Element} Centered loading spinner with text.
 */
export const Loader: React.FC<LoaderProps> = ({ text = 'Generating...' }) => {
  return (
    <div className="flex flex-col items-center justify-center space-y-4 p-8 text-center">
      <svg className="animate-spin h-10 w-10 text-brand-accent" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
      </svg>
      <p className="text-lg font-semibold text-brand-text-secondary">{text}</p>
    </div>
  );
};
